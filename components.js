...
export const Components = {
        ...
        return `
            <div class="post-card glass-panel animate-fade" data-post-id="${post.id}" style="margin-bottom:15px;">
                ...
                <div class="post-content" style="margin-bottom:15px;">
                    <p style="margin:0; line-height:1.6; color:var(--text-main); white-space:pre-wrap; font-size:1.05rem;">${post.content}</p>
                </div>
                ...
                <div class="post-stats" style="display:flex; flex-direction:column; gap:4px; padding:8px 0; border-top:1px solid var(--border-light);">
                    <div class="reactions-row-1" style="display:flex; gap:3px; flex-wrap:wrap; justify-content:space-between;">
                        <button onclick="window.App.handleReaction('${post.id}', 'like')" class="reaction-btn ${post.reactions.like > 0 ? 'active' : ''}" data-type="like" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            👍 ${post.reactions.like || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'dislike')" class="reaction-btn ${post.reactions.dislike > 0 ? 'active' : ''}" data-type="dislike" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            👎 ${post.reactions.dislike || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'heat')" class="reaction-btn ${post.reactions.heat > 0 ? 'active' : ''}" data-type="heat" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            🌶️ ${post.reactions.heat || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'wild')" class="reaction-btn ${post.reactions.wild > 0 ? 'active' : ''}" data-type="wild" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            🤪 ${post.reactions.wild || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'admire')" class="reaction-btn ${post.reactions.admire > 0 ? 'active' : ''}" data-type="admire" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            🤩 ${post.reactions.admire || 0}
                        </button>
                    </div>
                    <div class="reactions-row-2" style="display:flex; gap:3px; flex-wrap:wrap; justify-content:space-between;">
                        <button onclick="window.App.handleReaction('${post.id}', 'gross')" class="reaction-btn ${post.reactions.gross > 0 ? 'active' : ''}" data-type="gross" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            🤢 ${post.reactions.gross || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'wtf')" class="reaction-btn ${post.reactions.wtf > 0 ? 'active' : ''}" data-type="wtf" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            😱 ${post.reactions.wtf || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'dope')" class="reaction-btn ${post.reactions.dope > 0 ? 'active' : ''}" data-type="dope" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            🧊 ${post.reactions.dope || 0}
                        </button>
                        <button onclick="window.App.handleReaction('${post.id}', 'cap')" class="reaction-btn ${post.reactions.cap > 0 ? 'active' : ''}" data-type="cap" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            🧢 ${post.reactions.cap || 0}
                        </button>
                        <button onclick="window.App.showCommentModal('${post.id}')" class="reaction-btn" style="background:none; border:1px solid var(--border-light); border-radius:12px; padding:2px 6px; cursor:pointer; font-size:0.75rem; white-space:nowrap;">
                            💬 ${post.commentCount || 0}
                        </button>
                    </div>
                </div>
    ...
    sponsoredAd(ad) {
        const content = ad.content || ad.text || (typeof ad === 'string' ? ad : '');
        const mediaUrl = ad.media_url || ad.media;
        const mediaType = ad.media_type || ad.type || 'image';
        ...
        return `
            <div class="post-card glass-panel sponsored-ad animate-fade" style="border: 2px solid var(--primary-orange); background: rgba(255, 157, 0, 0.05); margin-bottom: 20px;">
                ...
                <div class="post-content" style="margin-top:10px;">
                    <p style="font-size:1.1rem; line-height:1.4; color:white;">${content}</p>
                    ${mediaUrl ? (mediaType === 'video' ? 
                        `<video src="${mediaUrl}" autoplay muted loop playsinline class="post-media" style="border-radius:12px; margin-top:10px; width:100%;"></video>` : 
                        `<img src="${mediaUrl}" class="post-media" style="border-radius:12px; margin-top:10px; width:100%;">`) : ''}
                </div>
                <div class="post-footer" style="margin-top:15px; display:flex; justify-content:center;">
                    <a href="${link}" target="_blank" class="btn-primary" style="width:100%; text-align:center; text-decoration:none; padding:12px; border-radius:30px; font-weight:bold; letter-spacing:1px; box-shadow: 0 0 20px var(--primary-orange-glow); color:white;">LINK YOUR MIND →</a>
                </div>
            </div>
        `;
    },
